...
|-------|-----------------|
| `UserPromptSubmit` | architect_enforcer.sh |
...
| **Purpose** | **Quality gate.** Runs validation checks before the `gitpro` skill executes: TypeScript type checking and toast usage checks. Blocks gitpro if validation fails. |
| **Mechanism** | Detects project type (Node/Python), runs type checker (`npm run check-types` or `pyright`/`mypy`), checks toast usage. Returns `permissionDecision: "deny"` on failure. |
| **Currently Blocking?** | Yes -- hard blocker with deny decisions. Timeout is 120s (the longest of any hook). |
| **Async Candidate?** | **NO** |
| **Reasoning** | This is a pre-commit quality gate. Its entire purpose is to block the gitpro skill from creating a commit if there are type errors. If async, gitpro would proceed with the commit while type-checking runs in the background, defeating the validation purpose. However, the 120s timeout is noteworthy -- this is the slowest hook and would benefit most from being faster, but its blocking nature is essential. |
...
| 2 | `block-db-commands.sh` | PreToolUse (Bash) | Block DB migrations | **Hard blocker (deny)** | **NEVER** | Safety-critical; async = no protection |
| 3 | `git-guard.sh` | PreToolUse (Bash) | Enforce gitpro, block destructive git | **Hard blocker (deny)** | **NEVER** | Safety-critical; most comprehensive guard |
| 4 | `dev-server-guard.sh` | PreToolUse (Bash) | Block dev server management | **Hard blocker (deny)** | **NEVER** | Safety-critical; prevents server conflicts |
| 5 | `block-console-log.sh` | PreToolUse (Edit/Write) | Enforce Pino over console.log | **Hard blocker (deny)** | **No** | Must prevent bad code from being written |
| 6 | `pre-gitpro.sh` | PreToolUse (Skill) | Pre-commit quality gate | **Hard blocker (deny)** | **No** | Must block commit if validation fails |
| 7 | `pre-commit` (git hook) | Git pre-commit | Block direct git commits | N/A (git hook) | N/A | Not a Claude Code hook |

---

## Recommendations

### 1. Do NOT make any PreToolUse hooks async
...
### 2. Do NOT make context-injection hooks async

`architect_enforcer.sh` injects context that must be available **before** Claude processes the current turn. Async delivery would delay the context by one turn, defeating its purpose.

### 3. Consider async for future informational hooks
...
### 4. Watch the `pre-gitpro.sh` timeout
